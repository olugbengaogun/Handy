use crate::audio_toolkit::{
    ...
    build_whisper_initial_prompt, detect_output_language, normalize_for_transcription,
    ...
    pub fn transcribe_with_language(
        ...
        let (result, output_language, model_languages) = {
            ...
            if let LoadedEngine::TranscribeCpp(session) = &engine {
                let model = session.model();
                let caps = model.capabilities();
                // Informational only: the run extension and fuzzy-correction
                // behavior are gated on `model_is_whisper` below because a
                // non-whisper architecture may advertise this capability yet
                // reject the whisper-kind extension.
        ...
        // reached the decoder, so they fall through to fuzzy correction. Upstream
        // passes model_is_whisper here; this fork's flag is the stricter test.
...
fn post_process_transcription_text(
    ...
    fail_open_text_transform(raw, |raw| {
        ...
        // answer". This pass keys off the trailing comma instead. It runs after
        // upstream's filler pass rather than instead of it: one strips single
        // words ("um"), the other whole clauses, and they are independently
        // toggleable settings.
